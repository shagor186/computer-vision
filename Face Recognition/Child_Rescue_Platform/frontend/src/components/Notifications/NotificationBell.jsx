...
const NotificationBell = ({ unreadCount, onNotificationClick }) => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://127.0.0.1:5000/api/notifications?per_page=5', {
        headers: {
  ...
  const markAsRead = async (notificationId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://127.0.0.1:5000/api/notifications/${notificationId}/mark-read`, {}, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
