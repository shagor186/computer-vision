...
const Navbar = ({ user, onLogout, onMenuToggle, currentView, onNavigate }) => {
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      if (!token) {
        console.warn('No token found, skipping notification count fetch');
        return;
      }

      const response = await fetch('http://127.0.0.1:5000/api/notifications?unread_only=true&per_page=1', {
        headers: {
  ...
  const handleNotificationClick = () => {
    ...
    // Refresh count when notifications are viewed
